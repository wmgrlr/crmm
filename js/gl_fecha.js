...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.gl_fecha = global.gl_fecha || {}));
...
/*BEGIN test_fecha_s*/
/****************************************************/
function numero_dias_mes(inp_date)
{
  let month_day = [29,31,28,31,30,31,30,31,31,30,31,30,31];
  let year = 1;
  let month = 1;
  let day = 1;
  let s_year = "    ";
...
function crearCalendario(year, month) {
      let date = new Date(year, month - 1, 1); // month - 1 porque JavaScript cuenta los meses desde 0
      const firstDay = date.getDay();
      const lastDay = new Date(year, month, 0).getDate();
      const dias = ['Dom', 'Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab'];
      let diasGuardia = [];
      var  diasGuardiaResul = [];
      let html = '<table>';
      html += '<thead><tr>';
      html += dias.map(dia => `<th>${dia}</th>`).join('');
      html += '</tr></thead>';
      html += '<tbody>';
      let currentDay = 1;
      let week = '<tr>';
      ...
      function addDays(date, days, primerDia, segundDia, mes) {
         let newDate = new Date(date.getTime());
         newDate.setDate(date.getDate() + days);
         diasGuardiaResul.push(newDate, newDate.getDate(), newDate.getMonth() );

        while ( (newDate.getDate() <= segundDia) && (newDate.getMonth() === mes) ) {
         
          return addDays(newDate, 4, primerDia, segundDia, mes);
         
        }
        
        return [newDate, newDate.getDate(), newDate.getMonth()  ];
    }

     diasGuardia = addDays(currentDate, 4, date.getDate(), lastDay, (month - 1) );
      ...
      while (currentDay <= lastDay) {
        ...
        currentDay++;        
        let dayPass = date.getDay();
        if (dayPass === 6) {
          week += '</tr>';
          html += week;
          week = '<tr>';
        } 
       let nuevoDia = date.getDate() + 1;              
	...
	if (n_month < 10)
	 {
      ...
      s_month = n_month.toString();  
    } 
	 
	 if (n_day < 10)
	 {
	  s_day = "0"+n_day.toString();
	 }
     else{
      s_day = n_day.toString();   
     }
	
	return n_year+s_month+s_day;
}
/****************************************************/
/*END prepare_output                                */
/****************************************************/
...
function prepare_output_for (n_year,n_month,n_day,formato){
	...
	if (n_month < 10)
	 ...
	 switch (formato) {
        case    'DD/MM/YYYY':  return s_day+'/'+s_month+'/'+n_year;
        case    'DD/MM/YY':    return s_day+'/'+s_month+'/'+n_year;
		case    'DDMMYYYY':    return s_day+s_month+n_year;
      }
...
/****************************************************/
/****************************************************/
/*BEGIN contar_fecha_for*/
/****************************************************/
function contar_fecha_for( inp_date, number_day, formato)
{
var  s_year;
var  s_month;
var  s_day;
...
out_date = prepare_output_for (n_year,n_month,n_day,formato);

return (out_date);
}  /*  contar_fecha_for  */
/****************************************************/
/*END contar_fecha_for*/
/****************************************************/
...
exports.contar_fecha = contar_fecha;
exports.contar_fecha_for = contar_fecha_for;
exports.dia_semana = dia_semana;
exports.meses_entre = meses_entre;
exports.numero_de_dias = numero_de_dias;
exports.numero_dias_mes = numero_dias_mes;
exports.crearCalendario = crearCalendario;
